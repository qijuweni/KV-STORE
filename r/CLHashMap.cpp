...
void* CLHashMap::Delete(void *ptr)
{
	...
	if(pNode == NULL)
	{
		pPara->pTask->pPackage->m_RequestHead.valueLen = -1;
	 ...
	 if(pMap->m_pbucketHeadArray[index] != pNode)
	 {
		 pMap->m_pLogger->WriteLogger(&(pNodePre->pNodeNext), sizeof(SLbucktNode*));	
		 pNodePre->pNodeNext = pNode->pNodeNext;
	 }
	 else
	 {
		pMap->m_pLogger->WriteLogger(&(pMap->m_pbucketHeadArray[index]), sizeof(SLbucktNode*));
		pMap->m_pbucketHeadArray[index] = pNode->pNodeNext;
	 }

	 pMap->m_pStoreManager[pPara->threadIndex]->Delete(pNode->value);
	 pMap->m_pStoreManager[pPara->threadIndex]->Delete(pNode);
...
//		goto next;
		pMap->m_pStoreManager[pPara->threadIndex]->ClearLogger();
		delete pPara->pTask->pPackage->pValue;
		pPara->pTask->pPackage->pValue = NULL;
		pPara->pTask->pPackage->m_RequestHead.valueLen = -1;
		write(pPara->pMap->m_pipeWriteFd, &pPara->pTask, sizeof(SLTask*));
		return NULL;
	}
...
void* CLHashMap::Find(void *arg)
{
	...
	if(pNode == NULL)
	{
		pPackage->m_RequestHead.valueLen = -1;
